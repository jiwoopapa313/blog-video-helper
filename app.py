# -*- coding: utf-8 -*-
# app.py — 블로그·유튜브 통합 생성기 (한국어 고정 + 이미지 EN only + 안정모드 + 사람맛 강화 + 본문+해시태그 일괄 복사)
...
st.caption(f"KST {datetime.now(KST).strftime('%Y-%m-%d %H:%M')} · 한국어 고정 · 이미지 EN 전용(no text) · 안정 모드 · Vrew 일괄복사 · 본문+해시태그 일괄 복사")

# components.html key 지원 체크
try:
...
def humanize_ko(text: str, mode: str, region: str = "관악구", persona: str = "강쌤") -> str:
    ...
    return chat_cached(style_sys, ask, st.session_state.get("model_text","gpt-4o-mini"), 0.6)
...
def gen_youtube(topic, tone, n, mode):
    sys = (
      "[persona / voice rules]\n"
      "- 화자: 20년 경력의 현장 전문가 “강쌤”. 말투는 차분, 가끔 유머. 존대.\n"
      "- 문체: 짧은 문장과 긴 문장을 섞어 리듬감. 2~3문장마다 호흡.\n"
      "- 사례: 최소 1개. 비교와 주의 포인트.\n"
      "- 마무리: 2줄 요약 + 행동 체크리스트(3~5개). 영업형은 마지막 1줄만 CTA.\n\n"
      ...
      "SEO titles (10, Korean) must include the main keyword early, use numbers/brackets wisely, avoid clickbait.\n"
      "Provide exactly N chapters (3~5 sentences each). All visuals depict Korean context."
    )
    user = (
      f"[topic] {topic}\n"
      f"[tone] {tone}\n"
...
def gen_blog(topic, tone, mode, min_chars, img_count):
    sys = (
      "[persona / voice rules]\n"
      "- 화자: 20년 경력의 현장 전문가 “강쌤”. 말투는 차분, 가끔 유머. 존대.\n"
      "- 문체: 짧은 문장과 긴 문장 섞기. 2~3문장마다 호흡. 현장 디테일 1~2개.\n"
      "- 사례/비교/주의/대안 포함. 마무리 2줄 요약 + 체크리스트 3~5개.\n\n"
...
st.markdown("---")
st.caption("한국어 고정 · 이미지 EN only(no text) · 안정 모드 · 사람맛 강화 · Vrew 일괄복사 · 본문+해시태그 일괄 복사 · .txt/.md 저장 지원")
